---
title: "Meet Anuradha"
slug: "bio"
---

![Anuradha Weeraman](/images/anuradha-weeraman.jpg)

### I'm a Technologist, Software Architect, [CTO](https://www.linkedin.com/in/anuradha-weeraman/), [Debian Developer](https://qa.debian.org/developer.php?login=anuradha), Member of the [Forbes Technology Council](https://councils.forbes.com/profile/Anuradha-Weeraman-Co-Founder-Chief-Technology-Officer-Verdentra/1f9ab90d-e656-4043-8569-988cf403c237)


I specialize in architecting and delivering software solutions that deliver business outcomes. As Co-Founder and CTO of [Verdentra](https://verdentra.com), I lead the firm's commitment to excellence in product engineering, strategic consulting, and trusted advisory services.

I have built and led global engineering teams, delivering technology solutions that span AI, IoT, mobile, Web3, blockchain, digital assets, cryptography, and distributed systems. I have spearheaded successful product engineering initiatives across diverse sectors and industries. A strong advocate for Software Freedom, I contribute to the [Linux kernel](https://kernel.org/) and [Debian](https://debian.org/) communities.

---

## Selected Engagements

25 years of technology leadership across financial services, healthcare, IoT, and emerging technology.

| Domain | Engagement | Scale | Outcome |
|--------|------------|-------|---------|
| Financial Services | Payment infrastructure modernization | 900+ member engineering organization | Payments integration with ISO20022 |
| Financial Services | PSD2 open banking compliance | 15-month regulatory deadline for a Fortune 500 client | Delivered ahead of mandate |
| Financial Services | Entitlements platform modernization | VSAM → DB2 migration for a Fortune 500 client | API-enabled legacy systems for downstream modernization |
| AdTech | Blockchain consortium platform | 10-month startup incubation | MVP launched with founding consortium partners |
| Insurance/IoT | Connected worker safety platform | Edge-to-cloud, 24 months | Real-time incident detection via wearable sensors |
| Healthcare | AI-powered health advisor | Innovation lab build | Gold Stevie Award — Best Healthcare Technology |
| Healthcare | Smart medication adherence platform | IoT + ML | Smart blister pack and assistant appliance, cloud platform, patent contribution |
| Enterprise Software | Records management platform | Fortune 500 client | Resolved multi-year stability issues, achieved HIPAA compliance |
| Enterprise Software | Middleware platform port | Fortune 500 client | Expanded vendor reach to mainframe and Unix markets (4+ operating systems) |

---

## Track Record

**Founder & Technology Executive**: Co-Founder and CTO of Verdentra, a consulting startup delivering product engineering, strategic consulting, and trusted advisory services.

**Enterprise Architecture at Scale**: Technology governance and delivery assurance for large distributed engineering organizations. Lead architect across a 900+ member engineering organization serving a Big Four US bank.

**Regulatory & Compliance Delivery**: Architected PSD2/open banking and HIPAA-compliant solutions under hard deadlines. Track record: zero missed compliance dates.

**0→1 Product Engineering**: Built AI and IoT products from concept to market — including a Gold Stevie Award-winning healthcare AI and a patent-contributing connected health platform.

---

## Credentials

**Recognition**
- US Patent 11,604,662 B2 — Co-inventor, UI modernization in computing environments
- Inductee, Virtusa Winners Club for Engineering and Technology Excellence
- Gold Stevie Award — "Best New Product or Service of the Year (Software/Healthcare Technology)"
- Virtusa "Idol" Hackathon - Winner
- Citi Mobile Challenge EMEA 2015 — Finalist

**Certifications**
- Software Architecture Professional Certificate — Software Engineering Institute, Carnegie Mellon University, 2019
- Advanced Reinforcement Learning — MIT Professional Education, 2025
- TOGAF 9.1 — The Open Group, 2015
- Certified Scrum Master (CSM) — Scrum Alliance, 2014
- Artificial Intelligence Nanodegree — Udacity, 2017
- Deep Reinforcement Learning Nanodegree — Udacity, 2019
- Control of Mobile Robots - Georgia Tech (Coursera), 2016

**Memberships**
- Forbes Technology Council — Member
- Open Source Initiative — Member
- Debian Project — Developer & Package Maintainer

---

## Giving Back

**4th Angel Mentoring Program** — Cleveland Clinic

Trained volunteer mentor in Cleveland Clinic's cancer support initiative, providing one-on-one guidance to patients and caregivers navigating their cancer journey. Focused on empowering individuals with knowledge, emotional support, and practical resources.

**Open Source Contributions**
- Debian Linux — Package maintainer for AT&T KornShell (ksh, ksh93u+m) and system utilities
- Linux Kernel — Contributor
- Apache Usergrid — Contributor

**Community**
- VirtusaX 2014 — Organized startup hackathon in partnership with SLASSCOM
- Mentorship — Weekly office hours for students and early-career technologists

---

[Schedule a Call](https://cal.com/anuradha) · [GitHub](https://github.com/aweeraman) · [Twitter](https://twitter.com/anuradha) · [LinkedIn](https://www.linkedin.com/in/anuradha-weeraman) · [YouTube](https://www.youtube.com/channel/UCCJaXCP9hRNbJ5az1PHOhtw?view_as=subscriber) · [Keybase](https://keybase.io/aweeraman) · [PGP Key](https://keys.openpgp.org/vks/v1/by-fingerprint/F56B8F14E014CDEF5D047FC1636DB5A1D91860FD)
